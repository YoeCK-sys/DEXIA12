...
const ParticleBackground = () => {
  ...
  useEffect(() => {
    ...
    const createParticles = () => {
      ...
      for (let i = 0; i < particleCount; i++) {
        particles.push({
          ...
          size: Math.random() * 3 + 2 // Increased particle size
        })
      }
    }
    ...
    const drawParticles = () => {
      ...
      ctx.strokeStyle = 'rgba(52, 152, 219, 0.3)' // Reduced opacity
      ...
      particles.forEach((particle, i) => {
        ...
        ctx.fillStyle = 'rgba(52, 152, 219, 0.7)' // More opaque particles
        ctx.fill()
    ...
    const drawConnections = (particle: typeof particles[0], index: number) => {
      particles.forEach((otherParticle, i) => {
        if (i > index) {
          ...
          if (distance < 150) { // Increased connection distance
            ...
            ctx.globalAlpha = 1 - distance / 150 // Fade out distant connections
            ctx.stroke()
            ctx.globalAlpha = 1
          }
        }
      ...
      if (distance < 200) {
        ...
        ctx.globalAlpha = 1 - distance / 200 // Fade out distant mouse connections
        ctx.stroke()
        ctx.globalAlpha = 1
      }
    }
